...
const CartSection = ({ Product, cartProduct, setCartProduct }) => {
    const { name, description, price, period, tagType, features, icon, id } = Product;
    const handelChooseCart = (Product) => {
        // console.log(Product);
    ...
    return (
        <div key={id} className="bg-base-100 shadow-sm">
            <div className="card-body">
                ...
                <div>
                    ...
                    <button className='btn w-full bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white rounded-full mt-4'
                        onClick={() => handelChooseCart(Product)}>Buy Now</button>
                </div>
            </div>
        </div>
    );
